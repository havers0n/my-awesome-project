...
Модуль “Прогноз продаж” реализован как отдельная страница [/sales-forecast](/sales-forecast) в клиентской части.
Он предоставляет аналитикам и операционному персоналу визуальный инструмент для получения и анализа прогноза спроса на товары.

**Функционал:**
- Загрузка и отображение тренда прогнозных продаж (line chart)
- Список топ-продуктов по прогнозу (bar/progress)
- История всех полученных прогнозов с поиском, фильтрацией и пагинацией
- Кнопка “Запросить прогноз” (отправляет POST-запрос на backend, обновляет все виджеты страницы)
- Обработка загрузки, ошибок, edge-кейсов (нет данных, большая история и пр.)

**Архитектурные детали:**
- Вся бизнес-логика реализована на хуках (useEffect, useState), типы данных — в `src/pages/types.ts`
- Все API-запросы абстрагированы в модуле `src/api/forecast.ts`
- Данные (пока) работают с мок-эндпоинтом на backend, полностью готовы к интеграции с реальным API кассы
- UI полностью согласован с фирменным стилем и стандартами Tailwind

**TODO (дальнейшие шаги):**
- Переключить загрузку данных на реальные эндпоинты, когда будет готов API кассы
- Реализовать историю прогнозов через базу данных (см. sales_forecast_history)
- (Опционально) Экспорт в Excel, улучшения UX

**Быстрый старт (для тестирования):**
- Запусти фронтенд, перейди на /sales-forecast
- Можно тестировать все сценарии (пустые данные, ошибки, большая история)
- Для реальных данных — просто поменяй эндпоинты в `api/forecast.ts`
