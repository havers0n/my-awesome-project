...
export const getOrganizations = asyncHandler(async (req, res, next) => {
  console.log('🔍 getOrganizations called, supabaseAdmin type:', typeof supabaseAdmin);
  ...
  console.log('✅ Organizations query successful, data length:', data?.length);
  res.json(data);
});
...
export const createOrganization = asyncHandler(async (req, res, next) => {
  ...
  logAdminAction('create_organization_success', { adminId: (req as any).user?.id, orgId: data.id, orgName: data.name });
...
export const updateOrganization = asyncHandler(async (req, res, next) => {
  ...
  logAdminAction('update_organization_success', { adminId: (req as any).user?.id, orgId: data.id });
  res.json(data);
});
...
export const deleteOrganization = asyncHandler(async (req, res, next) => {
  ...
  logAdminAction('delete_organization_success', { adminId: (req as any).user?.id, orgId: id });
  res.status(204).send();
});
...
export const handleSupabaseError = (err: any, req: Request, res: Response, next: NextFunction) => {
    if (err.code && typeof err.code === 'string') { // Basic check for Supabase/PostgREST error
        logAdminAction('supabase_error', { adminId: (req as any).user?.id, code: err.code, message: err.message, path: req.path });
